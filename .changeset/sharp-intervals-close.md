...
- **Endpoints are instants.** Each needs an offset (`Z` or `±HH:MM`), may carry a `[Zone]` annotation, and may name a different zone from the other. They compare by epoch nanoseconds, as a TC39 `Temporal.Instant` does. Leap seconds and `[u-ca=…]` calendar annotations are rejected. Where a realm needs local-calendar edges, build them with `floorToZone` or `bucketRange` first.
...
The older positional functions in `plain/`, `utc/`, `zoned/` and `unix/` `interval/` keep their model. Most of them use a closed `[start, end]` model, and their documentation now says which model each one follows. Their fixes at the edges of the representable range are listed separately.

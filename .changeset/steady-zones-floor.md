...
- **`getHoursInZonedDay`** returns Temporal's `hoursInDay`, so a day whose midnight is skipped (America/Santiago, 8 September 2024) is 23 hours, not 24. **`mapZonedHoursInDay`** with no `disambiguation` or `offset` stops at the next local day instead of spilling into it.
- A transition that reopens the previous local date is its own bucket everywhere: `America/Goose_Bay` fell back at 00:01 on 7 November 2010, so `startOfZoned("2010-11-06T23:30:00-04:00[America/Goose_Bay]", "day")` returns `23:01`, a 59-minute day, consistent with `floorToZone`, `bucketRange` and `intervalCountZoned`.

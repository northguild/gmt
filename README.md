...
- **~40× more CI test executions than all four competitors combined**: 1,002,960 from 33,432 tests run in all 10 timezones × 3 Node versions, vs. their combined 20,190.
- **~69× more test cases than `@internationalized/date`**: 33,432 vs. 386 — Adobe's own library, run at its own commit.
...
- `convertUtcToUnix()` and `convertUnixToUtc()` for explicit unix conversion.
- `convertZonedToUtc()` and `convertUtcToZoned()` for timezone-safe conversion.
...
`@northguild/gmt` exports every public function, type and regex as a flat named export of the package root, beside `Temporal` re-exported from `@js-temporal/polyfill`. The same exports are grouped into subpaths — `@northguild/gmt/calendar`, `duration`, `instant`, `interval`, `plain`, `precision`, `regex`, `span`, `types`, `unix`, `utc` and `zoned` — and, except for `regex` and `types`, into module subpaths such as `@northguild/gmt/plain/calculate`.
...
| ------------------------------- | -------------------------------------------------- | ------------------------------ | ------------------------------------ | ----------------------------------------- | -------------------------------- |
| Test files                      | 657                                                | 6                              | 58 / 60<br>(2 didn't run<br>locally) | 256                                       | 191<br>(52 core +<br>139 locale) |
| Individual test cases           | **33,432**                                         | 386                            | 1,222                                | 3,213                                     | 3,901                            |
| Effective CI test<br>executions | **1,002,960**<br>(33,432 × 3 Node<br>× 10 timezones) | 386<br>(×1 Node)               | 4,888<br>(1,222 × 4 Node)            | 3,213<br>(×1 Node)                        | 11,703<br>(3,901 × 3 Node)       |
...
**Result:** 33,432 tests across 657 files that exercise real behavior differences without redundant permutations. They run in CI as 1,002,960 executions — every one of them × 3 Node versions × 10 timezones.
...
| GMT's effective CI test<br>executions exceed all four<br>competitors **combined**<br>by ~40×                                                  | 1,002,960 vs. 386 + 4,888 + 3,213<br>+ 11,703 = 20,190                                                                                  |

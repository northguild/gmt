...
- **~15× more CI test executions than all four competitors combined**: 375,774 from 19,295 tests, the library's 18,741 of them run in all 10 timezones × 2 Node versions, vs. their combined 20,190.
- **~40× more test cases than `@internationalized/date`**: 19,295 vs. 386 — Adobe's own library, run at its own commit.
...
`@northguild/gmt` currently exports top-level `Temporal`, `duration`, `instant`, `plain`, `precision`, `span`, `zoned`, `unix`, `utc`, and `regex` namespaces, with direct subpath imports available under `@northguild/gmt/*`.
...
| ------------------------------- | -------------------------------------------------- | ------------------------------ | ------------------------------------ | ----------------------------------------- | -------------------------------- |
| Test files                      | 627                                                | 6                              | 58 / 60<br>(2 didn't run<br>locally) | 256                                       | 191<br>(52 core +<br>139 locale) |
| Individual test cases           | **19,295**                                         | 386                            | 1,222                                | 3,213                                     | 3,901                            |
| Effective CI test<br>executions | **375,774**<br>(18,741 × 2 Node<br>× 10 timezones,<br>+ 477 × 2 Node) | 386<br>(×1 Node)               | 4,888<br>(1,222 × 4 Node)            | 3,213<br>(×1 Node)                        | 11,703<br>(3,901 × 3 Node)       |
...
**Result:** 19,295 tests across 627 files that exercise real behavior differences without redundant permutations. They run in CI as 375,774 executions — the library's 18,741 tests × 2 Node versions × 10 timezones, plus 477 in `apps/dox` × 2 Node versions.
...
| GMT's effective CI test<br>executions exceed all four<br>competitors **combined**<br>by ~15×                                                  | 375,774 vs. 386 + 4,888 + 3,213<br>+ 11,703 = 20,190                                                                                  |

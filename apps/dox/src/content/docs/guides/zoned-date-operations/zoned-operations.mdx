...
Their `disambiguation` and `offset` options are deprecated and ignored: a boundary is a
real instant, exactly as TC39's `startOfDay()` takes no such options. Resolution

...
Every mode publishes a plan and records what happened, and every mode measures the gap the same way — yet each realm here was about to define "late" for itself. A flight is on time under US reporting when it arrives less than 15 minutes after schedule; a rail passenger's compensation keys on 60- and 120-minute thresholds; an ocean carrier's schedule reliability is measured in days. The arithmetic is identical; only the tolerance differs, and the tolerance is the caller's fact.
...
([14 CFR 234.2, "on-time"](https://www.law.cornell.edu/cfr/text/14/234.2), [DCSA JIT Port Call OpenAPI 1.2.0-Beta-2, `eventClassifierCode`](https://raw.githubusercontent.com/dcsaorg/DCSA-OpenAPI/master/jit/v1/jit_v1.2.0-Beta-2.yaml), [DCSA `EVENT_DOMAIN` 1.0.4 (Track & Trace 2.2)](https://api.swaggerhub.com/domains/dcsaorg/EVENT_DOMAIN/1.0.4), [Regulation (EU) 2021/782 Art. 19, rail delay compensation thresholds](https://eur-lex.europa.eu/eli/reg/2021/782/oj))
...
- `packages/gmt/src/transport/compare/scheduleDeviation.ts`:
  ...
  - `classifyPunctuality(planned: string, actual: string, tolerance: { late: string, early?: string }): 'early' | 'onTime' | 'late'` — Half-open at the late boundary: exactly `late` is late, matching "less than 15 minutes" in 14 CFR 234.2.
...
- `packages/gmt/src/transport/get/timestampClass.ts`:
  ...
  - `estimateDrift(events): { first: string, last: string, drift: string, revisions: number } | null` — How far the estimate moved between first and last `EST`, for schedule-reliability reporting.
...
- **Tolerances are parameters; the regulatory values are documentation.** 15 minutes (14 CFR 234.2), 60 and 120 minutes (Regulation 2021/782) and the maritime day-based conventions appear in the JSDoc with citations so a caller can pick one knowingly; none is a default, because "on time" without a stated tolerance is not a measurement.
...
- OTP aggregation is included because the definition is regulatory and small; anything beyond a rate — percentiles, causal attribution — is the consumer's.
...
- `classifyPunctuality` with `late: 'PT15M'` returns `'onTime'` at 14 minutes 59 seconds and `'late'` at exactly 15 minutes — the 14 CFR 234.2 boundary, asserted
...
- `estimateDrift` over three `EST` revisions reports `revisions: 3` and the first-to-last difference

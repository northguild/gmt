...
**Scope:** Consistent date shifting for research data sets, with an age cap the caller sets.
...
Two constraints make it non-trivial. The shift must be **identical within a patient and different across patients**, or intervals break or re-identification becomes possible. And de-identification schemes cap reported ages above a threshold, because a birth year plus a rare age is identifying.
...
- `packages/gmt/src/health/calculate/ageCap.ts`:
  - `capAge(age: { value: number, unit: string }, options: { threshold: number }): { value: number, unit: string, capped: boolean } | null` — Aggregates ages above the threshold the caller states; a missing threshold returns the sentinel.
...
- Any de-identification scheme has identifier categories beyond dates and ages. This story addresses dates and ages only, and the JSDoc must not imply it delivers compliance with any scheme.

## Corrections

- `capAge` defaulted its threshold to one scheme's number; the threshold is required and the function returns the sentinel without it.
...
- `capAge` with `threshold: 89` returns `capped: true` for 92 and `false` for 89; with `threshold: 85` age 86 is capped; a missing threshold returns the sentinel

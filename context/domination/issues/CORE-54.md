...
The observance shift is where implementations diverge. US federal law moves a Saturday holiday to the preceding Friday and a Sunday holiday to the following Monday; the UK substitutes the next working day; Japan moves a Sunday holiday to Monday. None of these is a default, so the shift is a parameter.

([5 U.S.C. §6103(a)–(b)](https://www.law.cornell.edu/uscode/text/5/6103), [ECB TARGET closing days](https://www.ecb.europa.eu/paym/target/target2/profuse/calendar/html/index.en.html) as a worked rule set, [Meeus, *Astronomical Algorithms*, ch. 8, Date of Easter](https://www.willbell.com/math/mc1.htm))
...
- **Rules are logic; rule sets are data.** This story ships the three rule forms and the expansion. It ships no country's rule set on the default import path; FIN-41's opt-in `finance/data` subpath is where reference rule sets live, each with its source and revision. The US federal list and the TARGET closing days are documentation examples in the JSDoc, cited, not exported constants.
- **Observance has no default.** `fixed` rules without `observed` fall on the day they fall; a caller who wants the US shift says so. Defaulting to the US convention would silently make every non-US calendar wrong.
- **Easter is a formula with two calendars.** The Gregorian computus (Meeus ch. 8, Oudin's algorithm) and the Julian computus give different Sundays most years; Orthodox holidays (Greek, Russian and Serbian markets, Cypriot and Romanian public holidays) key on the Julian one. The function is exact for every year in Temporal's range and needs no table.
...
- `nthWeekdayOfMonth(2026, 1, 1, 3)` returns the third Monday of January 2026 (MLK Day); `n: 'last'` for May returns Memorial Day; a fifth Friday in a month with four returns the sentinel
...
- With `nextWorkingDay` and a calendar already containing 26 December, Christmas 2021 (a Saturday) is observed on Tuesday 28 December, asserted against the UK substitute days
...
- `buildBusinessCalendar` from the eleven 5 U.S.C. §6103 rules for 2026 produces a calendar on which `isBusinessDay` is false on every listed date and their observed shifts, and true on the surrounding days

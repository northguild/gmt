...
- **Settlement** — DEMURRAGE (no. 30) "shall not be subject to exceptions which apply to Laytime unless specifically stated in the Charter Party" — the codified form of *Dias Compania Naviera v Louis Dreyfus* [1978] 1 WLR 261 (HL): "once on demurrage, always on demurrage". DESPATCH is owed for time saved, and the two despatch clauses differ by exactly the excepted periods: ON ALL WORKING TIME SAVED (no. 32) runs to laytime expiry "excluding any periods excepted from the Laytime"; ON ALL TIME SAVED (no. 33) "including periods excepted".
...
([BIMCO Laytime Definitions for Charter Parties 2013, nos. 5, 8–10, 14–33](https://www.bimco.org/media/qy2neqim/laytime-definitions-for-charter-parties-2013-v2.pdf), [Gencon 1994 Part II clause 6(c) and clause 7](https://consemaracademy.com/wp-content/uploads/2024/01/GENCON-1994.pdf), [Dias v Louis Dreyfus, Lord Diplock at 263–264](https://www.trans-lex.org/302840/))
...
- `packages/gmt/src/maritime/calculate/laytimeUsed.ts`:
  - `laytimeUsed(start: string, end: string, options: { timeZone: string, dayDefinition: 'day' | 'calendarDay' | 'conventionalDay', exceptions: LaytimeException[], calendar?: BusinessCalendar, unlessUsed?: Interval[], onDemurrageFrom?: string }): { used: string, excluded: string, periods: Interval[], onDemurrage: string } | null` — Time counted, time excluded, and — after laytime expires — time on demurrage, to which exceptions no longer apply (BIMCO no. 30) unless the caller says the charter states otherwise.
...
- **Weather exceptions are observations, not predictions.** GMT cannot know when weather stopped work; the caller supplies the intervals. This is the same principle that removed `customsClearance` (INT-13).
- **Once on demurrage, always on demurrage is the default, and it is cited.** After `onDemurrageFrom` (or the computed expiry) exceptions stop applying, per BIMCO no. 30 and *Dias*; a charter that says otherwise passes `demurrageExceptions: true`.

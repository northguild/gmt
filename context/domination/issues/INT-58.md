...
- **Site-wide contrast work was added to this story** (owner decision). The Billing Deadlines widget exposed accent text below the Dox 7:1 text rule, so every accent used as text now uses its `-ink` theme-role token, `--gmt-ice-dim` was retuned to at least 7:1 in both themes, and the Starlight badges were fixed. See `../../dox/reference/design-system.md` rule 6.
- **The tax-regulation citation in the `getFiscalPeriod` and `internal/fiscalCalendar` JSDoc stays for now** (owner decision). It predates this story and sits outside its intermodal scope, so it is left for a separate story rather than removed here.

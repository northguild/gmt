...
- **~55× more CI test executions than all four competitors combined**: 1,103,760 from 36,792 tests run in all 10 timezones × 3 Node versions, vs. their combined 20,190.
- **~95× more test cases than `@internationalized/date`**: 36,792 vs. 386 — Adobe's own library, run at its own commit.
...
**Result:** 36,792 tests across 673 files that exercise real behavior differences without redundant permutations. They run in CI as 1,103,760 executions — every one of them × 3 Node versions × 10 timezones.
...
| Individual test cases           | **36,792**                                         | 386                            | 1,222                                | 3,213                                     | 3,901                            |
| Effective CI test<br>executions | **1,103,760**<br>(36,792 × 3 Node<br>× 10 timezones) | 386<br>(×1 Node)               | 4,888<br>(1,222 × 4 Node)            | 3,213<br>(×1 Node)                        | 11,703<br>(3,901 × 3 Node)       |
...
| GMT's effective CI test<br>executions exceed all four<br>competitors **combined**<br>by ~55×                                                  | 1,103,760 vs. 386 + 4,888 + 3,213<br>+ 11,703 = 20,190                                                                                  |
...
the port. How many days are free, whether the day of discharge is free day one, whether weekends
count, and which clock a charge runs on are set by the carrier's tariff and the service contract.
So every term is a parameter, none has a default, and what comes back is what 46 CFR 541.6
requires a demurrage or detention invoice to state: the free time allowed, the dates it started
and ended, and the specific dates charged.
...
// Which two events a charge runs between: detention is gate-out to empty return.
const events = [
...
demurrageClock(events, "demurrage"); // { start: "2024-06-14T19:00:00Z", end: "2024-06-20T14:30:00Z" }
demurrageClock(events, "detention"); // { start: "2024-06-20T14:30:00Z", end: "2024-06-27T09:00:00Z" }
demurrageClock(events, "demurrage", { startEvent: "available" }); // { start: "2024-06-15T12:00:00Z", end: "2024-06-20T14:30:00Z" }
demurrageClock([{ type: "discharged", at: "2024-06-14T19:00:00Z" }], "demurrage"); // null (no gate-out yet)
```
...
- **Days are the terminal's local days.** `clockStart` and `clockEnd` are instants; their local
  ...
  `dwellTime(...).calendarDays` for the same dwell.
- **`firstDay` has no default.** `"eventDay"` makes the event day free day one; `"nextDay"` starts
  ...
  returns `null`.
- **`basis` decides which days count.** `"calendar"` counts every local day, so a weekend burns two
  free days. `"working"` counts only the working days of `options.calendar`, a `BusinessCalendar`
  (its weekend and holidays; its `timeZone` is not read), and suspends the clock on closed days
  after expiry too. It returns `null` without a calendar rather than assume a weekend.
...
- **`chargedDates` is the invoice's list.** 46 CFR 541.6 requires "the specific date(s) for which
  demurrage and/or detention were charged"; returning them makes the count auditable.
...
- **`demurrageClock` selects the pair.** Demurrage and storage run from `startEvent`
  (`"discharged"` by default, or `"available"` where the tariff starts at the availability date) to
  `gatedOut`; detention runs from `gatedOut` to `emptyReturned`, and `startEvent` is not read for
  it. Events may be in any order and every one is validated; a required event that is missing or
  present twice, or an end before its start, returns `null`. The result is an `Interval` in the
  caller's own strings, ready for `chargeableDays(start, end, …)`.

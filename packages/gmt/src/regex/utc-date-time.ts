 ...
 * seconds, in ISO 8601 extended format. The designator is an upper-case `Z`:
 * a lower-case `z`, which RFC 3339 §5.6 and Temporal also read, does not match (`rfc3339DateTime`
 * is the RFC 3339 pattern that accepts it).
 *
 ...
 * - No numeric offset — only the `Z` UTC designator is accepted.
 ...
 * captured.
 *
 * The written form only, with no RFC 9557 annotation. `isValidUtc` reads more: it matches this
 ...
 * @example utcDateTime.test("2024-03-15T14:30z")          // false (lower-case z)
...
export const utcDateTime: RegExp =
  /^(?:\d{4}|\+\d{6}|-(?!0{6})\d{6})-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])T(0[0-9]|1[0-9]|2[0-3]):[0-5][0-9](?::[0-5][0-9](?:[.,]\d{1,9})?)?Z$/;

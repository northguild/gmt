...
const discharged: ClockEvent = { type: "discharged", at: "2024-06-14T19:00:00Z" };
...
const emptyReturned: ClockEvent = { type: "emptyReturned", at: "2024-06-27T09:00:00Z" };
...
const emptyReleased: ClockEvent = { type: "emptyReleased", at: "2024-07-01T08:00:00Z" };
...
describe("demurrageClock", () => {
  ...
  `("selects $reads for $direction $scope", ({ direction, scope, startEvent, start, end }) => {
    const events = direction === "import" ? imported : exported;
    expect(demurrageClock(events, scope, { direction, startEvent })).toEqual({ start, end });
  });

  it("reads each leg from a list holding the whole cycle", () => {
    const cycle = [...imported, ...exported];
    expect(demurrageClock(cycle, "combined", IMPORT)).toEqual({ start: discharged.at, end: emptyReturned.at });
    expect(demurrageClock(cycle, "combined", EXPORT)).toEqual({ start: emptyReleased.at, end: loaded.at });
  });

  it("does not care about event order", () => {
    expect(demurrageClock([...imported].reverse(), "demurrage", IMPORT)).toEqual({
      start: discharged.at,
      end: gatedOut.at,
    });
  });
  ...
  it("echoes the caller's own strings, zone and all", () => {
    const local = { type: "gatedOut", at: "2024-06-20T10:30:00-04:00[America/New_York]" } as const;
  ...
  it("allows a clock that starts and ends at the same instant: an empty interval", () => {
    expect(
      demurrageClock([discharged, { type: "gatedOut", at: discharged.at }], "demurrage", IMPORT),
    ).toEqual({ start: discharged.at, end: discharged.at });
  });

  it("does not need the events the requested clock does not read", () => {
    expect(demurrageClock([discharged, gatedOut], "demurrage", IMPORT)).toEqual({ start: discharged.at, end: gatedOut.at });
    expect(demurrageClock([gatedOut, emptyReturned], "detention", IMPORT)).toEqual({ start: gatedOut.at, end: emptyReturned.at });
    expect(demurrageClock([gatedIn, loaded], "demurrage", EXPORT)).toEqual({ start: gatedIn.at, end: loaded.at });
  });

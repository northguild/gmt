...
describe("parseMonthFromUtc", () => {
  ...
  it.each`
    ...
    ${"2025-01-01T02:00:00Z"} | ${"UTC"}              | ${"01"}
    ${"2025-01-01T02:00:00Z"} | ${"America/New_York"} | ${"12"}
    ${"2025-01-01T02:00:00Z"} | ${undefined}          | ${"01"}
  `(
    "returns $expected for $value with timeZone $timeZone",
    ({ value, timeZone, expected }) => {
      expect(parseMonthFromUtc(value, { timeZone })).toBe(expected);
    },
  );
  ...
  `("returns the sentinel for invalid timeZone $timeZone", ({ timeZone }) => {
    expect(
      parseMonthFromUtc("2024-03-17T02:30:45.123456789Z", { timeZone }),
    ).toBe("");
  });
});

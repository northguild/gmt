...
describe("parseMillisecondFromUtc", () => {
  ...
  it.each`
    ...
    ${"2024-03-17T02:30:45.123456789Z"} | ${"UTC"}              | ${"123"}
    ${"2024-03-17T02:30:45.123456789Z"} | ${"America/New_York"} | ${"123"}
    ${"2024-03-17T02:30:45.123456789Z"} | ${undefined}          | ${"123"}
  `(
    "returns $expected for $value with timeZone $timeZone",
    ({ value, timeZone, expected }) => {
      expect(parseMillisecondFromUtc(value, { timeZone })).toBe(expected);
    },
  );
  ...
  `("returns the sentinel for invalid timeZone $timeZone", ({ timeZone }) => {
    expect(
      parseMillisecondFromUtc("2024-03-17T02:30:45.123456789Z", { timeZone }),
    ).toBe("");
  });
});

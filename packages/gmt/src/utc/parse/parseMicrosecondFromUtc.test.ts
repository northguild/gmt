...
describe("parseMicrosecondFromUtc", () => {
  ...
  it.each`
    ...
    ${"2024-03-17T02:30:45.123456789Z"} | ${"UTC"}              | ${"456"}
    ${"2024-03-17T02:30:45.123456789Z"} | ${"America/New_York"} | ${"456"}
    ${"2024-03-17T02:30:45.123456789Z"} | ${undefined}          | ${"456"}
  `(
    "returns $expected for $value with timeZone $timeZone",
    ({ value, timeZone, expected }) => {
      expect(parseMicrosecondFromUtc(value, { timeZone })).toBe(expected);
    },
  );
  ...
  `("returns the sentinel for invalid timeZone $timeZone", ({ timeZone }) => {
    expect(
      parseMicrosecondFromUtc("2024-03-17T02:30:45.123456789Z", { timeZone }),
    ).toBe("");
  });
});
